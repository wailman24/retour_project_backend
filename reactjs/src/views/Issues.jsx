...
export default function Issues() {
    ...
    //search
    const [search_piece, setSearch_piece] = useState("");
    const [search_description, setSearch_description] = useState("");
    ...
    const openDeleteIssue = (d) => {
        setIsOpenD(!isOpenD);
        setDeleteIssue(d);
    };

    const handleDelete = () => {
        axiosClient.delete(`/issues/${DeleteIssue.id}`).then(() => {
            setIsOpenD(!isOpenD);
            window.location.reload();
        });
    };
    ...
    const onUpdate = (ev) => {
        ev.preventDefault();
        axiosClient
            .put(`/issues/${issueUpdate.id}`, issueUpdate)
            .then(() => {
                setIsOpenU(!isOpenU);
                window.location.reload();
            })
                ...
                <div className="bg-white dark:bg-gray-900 relative shadow-md sm:rounded-lg overflow-hidden ">
                    <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                        <div className="flex items-center flex-1 space-x-4">
                            <h5>
                                <span class="text-gray-500">All Issues:</span>
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            {!loading && (
                                <tbody>
                                    {records
                                        .filter((d) => {
                                            return search_description.toLowerCase() ===
                                                ""
                                                ? d
                                                : d.description
                                                      .toLowerCase()
