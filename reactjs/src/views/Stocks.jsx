...
export default function Pieces() {
    ...
    const [search_serialnumber, setsearch_serialnumber] = useState("");
    ...
    const records = stocks.slice(firstIndex, lastIndex);
    const npage = Math.ceil(stocks.length / recordsPerPage);
    ...
    const handleDelete = () => {
        axiosClient.delete(`/stocks/${DeletePiece.id}`).then(() => {
            setIsOpenD(!isOpenD);
            window.location.reload();
        });
    };
    ...
    const onUpdate = (ev) => {
        ev.preventDefault();
        axiosClient
            .put(`/stocks/${pieceUpdate.id}`, pieceUpdate)
            .then(() => {
                setIsOpenU(!isOpenU);
                window.location.reload();
            })
                            ...
                            {!loading && (
                                <tbody>
                                    {records
                                        ...
                                        .filter((d) => {
                                            return search_serialnumber.toLowerCase() ===
                                                ""
                                                ? d
                                                : d.serial_number
                                                      .toLowerCase()
